...
* \y`\tracingonline=0`,
  ...
  \y`\tracinggroups=0`, 
  \y`\tracingassigns=0`,
  \y`\tracingfonts=0` (=`\pdftracingfonts` in pdf\TeX).
...
* \*\i numexpr `\numexpr<num. expression>`, \i dimexpr \*`\dimexpr<dimen expression>`.
  ...
  They are expandable in the context `\the\numexpr`.
...
* \x`\textdirection`, \x`\pardirection`, \x`\bodydirection`, \x`\pagedirection`:
  if they are nonzero then different direction (from default left to right,
  bottom to top) of typesetting material is used.
  For more information, see
  \ulink[https://ctan.gust.org.pl/tex-archive/macros/luatex/latex/luabidi/luabidi.pdf\#section.5]
  {chapter~5 of luabidi} documentation.
\enditems
