...
* the control sequence can be a \ii macro {\em macro}. When it is read, it is
  ...
  \ii expansion/process {\em expansion process} which ends when only printable text, primitive commands,
  registers, character constants, or font selectors remain.
\enditems
...
state} (the `-ini` option on the command line must be used). It already knows more than 700
primitive registers and primitive commands at this state.\fnote
{Roughly speaking, if you know all these primitive objects and the syntax of all
...
define it by
\i def `\def<character>{...}`.
If the name of a `<control sequence>`
listed in this document contains only one character,
then this character is also defined as an active character.
