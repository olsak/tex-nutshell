...
* the control sequence can be a \ii macro {\em macro}. When it is read, it is
  ...
  \ii expansion/process {\em expansion process} which ends when only printable tex, primitive commands,
...
state} (the `-ini` option on the command line must be used). It knows only about 300
...
in Lua\TeX/, because Lua\TeX/ contains
...
listed in this document, then it is also defined as active character.
...
\i leavevmode `\leavevmode`\fnote {The list is not fully completed, but most important commands
are mentioned here.}.
